...
function parseTipoMei(
  value: string | null,
): EntradaSimulacao["tipoMei"] | null {
...
export async function GET(request: NextRequest) {
  ...
  if (apiKeyError) {
    logger.error(
      "api.contadores.simulate.apikey",
      "Erro ao validar chave de API",
      { message: apiKeyError.message },
    );
    return NextResponse.json(
      { error: "Não foi possível validar a chave." },
      { status: 500 },
    );
  }
  ...
  const faturamentoAcumulado = parseNumberParam(
    request,
  ...
  const tipoMei = parseTipoMei(
    request.nextUrl.searchParams.get("tipoMei") ?? "geral",
  );
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Parâmetros obrigatórios: faturamentoAcumulado, mesAtual, cnae, folhaMensal, tipoMei.",
      },
      { status: 400 },
    );
  }
  ...
  if (!getCnae(normalizedCnae)) {
    return NextResponse.json(
      { error: "CNAE não reconhecido." },
      { status: 400 },
    );
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: quotaData, error: quotaError } = await (admin as any).rpc(
    "increment_quota",
    { p_api_key_id: key.id },
  );
  ...
  if (quotaError) {
    logger.error(
      "api.contadores.simulate.quota",
      "Erro ao contabilizar uso da chave",
      { message: quotaError.message },
    );
    return NextResponse.json(
      { error: "Não foi possível contabilizar o uso da chave." },
      { status: 500 },
    );
  }
